...
describe('MasterCard', function() {
  ...
  // Expect syntax is one way to do this, but ther  are others.
...
describe('Discover', function() {
  ...
  it('has a prefix of 6011 and a length of 19', function() {
    expect(detectNetwork('6011541234567890123')).to.equal('Discover');
  });
  it('has a prefix of 65 and a length of 16', function() {
    expect(detectNetwork('6511345678901234')).to.equal('Discover');
  });
  it('has a prefix of 65 and a length of 19', function() {
    expect(detectNetwork('6511541234567890123')).to.equal('Discover');
  });
...
describe('China UnionPay', function() {
  ...
  for (var length = 16; length <= 19; length++) {
    for (var i = 0; i < prefixes.length; i++) {
      (function(length, prefix) {
        it('has a prefix of ' + prefix + ' and a length of ' + length, function() {expect(detectNetwork(prefix + '4567123701' + addZeros(length, prefix.toString()))).to.equal('China UnionPay');
        });
      })(length, prefixes[i]);
    }
  }
  ...
  function addZeros(lengthNeeded, currentPrefix) {
    return ('0').repeat(lengthNeeded - currentPrefix.length - 10);
  }
});
describe('Switch', function() {
  ...
  for (var j = 0; j < lengths.length; j++) {
    for (var i = 0; i < prefixes.length; i++) {
      (function(length, prefix) {
        it('has a prefix of ' + prefix + ' and a length of ' + length, function() {expect(detectNetwork(prefix + '4567123701' + addZeros(length, prefix.toString()))).to.equal('Switch');
        });
      })(lengths[j], prefixes[i]);
    }
  }
  function addZeros(lengthNeeded, currentPrefix) {
    return ('0').repeat(lengthNeeded - currentPrefix.length - 10);
